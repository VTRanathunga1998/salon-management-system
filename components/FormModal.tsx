...
const FormModal = ({
  ...
  // Plain expression, not a locally-declared component — see the note
  // above DeleteConfirm for why that distinction matters here.
  const content =
    type === "delete" ? (
      <DeleteConfirm table={table} id={id} setOpen={setOpen} />
    ) : type === "create" || type === "update" ? (
      forms[table](setOpen, type, data, relatedData)
    ) : type === "convert" ? (
      forms[table](setOpen, "create", data, relatedData)
